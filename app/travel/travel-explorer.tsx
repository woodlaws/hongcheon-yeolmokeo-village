...
export function TravelExplorer() {
  ...
  return <>
    ...
    {visible.length > 0 ? <div className={styles.cardGrid}>{visible.map((place) => <article className={styles.card} key={place.id}>
      ...
      <div className={styles.cardBody}><p className={styles.type}>{place.type}</p><h3><a className={styles.cardHit} href={place.detailUrl} target="_blank" rel="noreferrer">{place.name}</a></h3><p className={styles.summary}>{place.summary}</p><dl><div><dt>추천 계절</dt><dd>{place.season}</dd></div><div><dt>추천 대상</dt><dd>{place.audience}</dd></div><div><dt>마을 기준</dt><dd>{place.distance}</dd></div><div><dt>예상 이동</dt><dd>{place.travelTime}</dd></div></dl><p className={styles.note}>{place.note}</p><div className={styles.cardActions}><a href={place.detailUrl} target="_blank" rel="noreferrer">자세히 보기 <ExternalLink size={14} /></a><a href={place.mapUrl} target="_blank" rel="noreferrer"><MapPin size={15} /> 지도에서 보기</a></div></div>
