...
import { groupAudiences, groupCases } from "@/data/group";
import { siteConfig } from "@/data/site";

export function GroupQuoteForm() {
  const params = useSearchParams();
  const [notice, setNotice] = useState("");
  const selectedProgram = groupAudiences.some((item) => item.id === params.get("program")) ? params.get("program") ?? "" : "";
  const selectedDuration = ({ day: "당일", overnight: "1박 2일", workation: "2박 3일" } as Record<string, string>)[params.get("duration") ?? ""] ?? "당일";
  const selectedCase = groupCases.find((item) => item.title === params.get("case"))?.title;
  ...
  return <form className="group-quote-form" onSubmit={submit}>
    ...
    <fieldset><legend>일정과 규모</legend><div>
      <label><span>희망 날짜 *</span><input name="preferredDate" type="date" min={today} required /></label>
      <label><span>대체 가능 날짜</span><input name="alternativeDate" type="date" min={today} /></label>
      <label><span>예상 인원 *</span><input name="guests" type="number" min="1" required placeholder="운영 가능 인원은 상담 후 확인" /></label>
      <label><span>희망 일정 *</span><select name="duration" required defaultValue={selectedDuration}><option>당일</option><option>1박 2일</option><option>2박 3일</option></select></label>
    </div></fieldset>
    <fieldset><legend>필요한 구성</legend><div>
      ...
      <label><span>관심 프로그램</span><select name="program" defaultValue={selectedProgram}><option value="">아직 정하지 못했어요</option>{groupAudiences.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
      <label className="full"><span>예상 예산</span><input name="budget" placeholder="총예산 또는 1인당 예산 · 협의가 필요하면 함께 적어주세요" /></label>
      <label className="full"><span>요청사항 *</span><textarea name="request" rows={6} required defaultValue={selectedCase ? `문의할 운영 사례: ${selectedCase}\n` : ""} placeholder="참가자 특성, 필요한 프로그램·식사·공간, 안전 고려사항을 알려주세요." /></label>
    </div></fieldset>
