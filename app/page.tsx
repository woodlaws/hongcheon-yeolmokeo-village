    ...
    <main>
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className="container hero-content">
          ...
          <div className="hero-actions"><Link href="/programs" className="button button-primary">숙소·프로그램 보기 <ArrowRight size={18}/></Link><Link href="/group" className="button button-ghost">단체 방문 문의</Link></div>
      ...
      <section className="value-section"><div className="container value-layout"><div className="value-copy"><p className="section-kicker">WHY YEOLMOKEO</p><h2>하루를 비우면,<br/>마음이 채워집니다</h2><p>홍천의 깨끗한 자연 속에서 나와 가족, 동료와 함께 진짜 쉼의 가치를 느껴보세요.</p><div className="value-list"><div><Trees/><span><b>청정 자연</b>맑은 계곡과 울창한 숲</span></div><div><BedDouble/><span><b>편안한 숙박</b>자연 속 아늑한 공간</span></div><div><Sprout/><span><b>맞춤형 프로그램</b>누구나 즐기는 치유 체험</span></div><div><Utensils/><span><b>지역 먹거리</b>홍천의 신선한 식재료</span></div></div></div><div className="value-collage"><Image className="collage-main" src="/images/stay-room.png" alt="햇살이 드는 열목어마을 숙소" width={850} height={900}/><Image className="collage-top" src="/images/quiet-time-3.png" alt="숲속 해먹에서 쉬는 모습" width={520} height={400}/><Image className="collage-bottom" src="/images/valley-rest.png" alt="열목어마을 맑은 계곡물" width={520} height={400}/></div></div></section>

      <section className="home-section container"><header className="section-heading"><p className="section-kicker">FOR YOU</p><h2>이런 분들께 추천하는 프로그램</h2><p>누구와 오시든, 열목어마을에서 특별한 쉼이 시작됩니다.</p></header><div className="recommend-grid">{programs.slice(0,3).map(program=><Link href={`/programs/${program.slug}`} className="recommend-card" key={program.slug}><Image src={program.image} alt={program.title} width={760} height={520}/><div><p>{program.audience}</p><h3>{program.title}</h3><span>{program.summary}</span><i><ArrowRight/></i></div></Link>)}</div></section>
