...
import { ArrowRight, Bus, CalendarDays, Car, Check, ChevronRight, CircleHelp, CloudSun, ExternalLink, HandHeart, Info, MapPinned, Mountain, Navigation, PackageCheck, ShieldAlert, Sparkles, Trees, Users } from "lucide-react";
...
export const metadata: Metadata = {
  ...
  openGraph: { title: "홍천 열목어마을 여행 안내｜1박 2일 코스·주변 관광지·오시는 길", description: "여행 일정부터 주변 관광지, 교통·준비물·안전정보까지 실제 방문 계획에 필요한 내용을 확인하세요.", url: pageUrl, type: "website", locale: "ko_KR", siteName: "홍천 열목어마을", images: [{ url: "/images/yeolmokeo-village-panorama-02.jpg", width: 1200, height: 630, alt: "산과 하천을 따라 집과 밭이 자리한 열목어마을 전경" }] },
};

const quickFacts = [
  { icon: MapPinned, label: "방문 주소", value: siteConfig.address },
...
export default function GuidePage() {
  ...
  return <><SiteHeader /><main className="travel-guide">
    ...
    <section className="guide-section guide-attractions" aria-labelledby="attractions-title"><div className="container"><div className="guide-heading"><p className="section-kicker">NEARBY PLACES</p><h2 id="attractions-title">열목어마을과 함께 둘러보기 좋은 곳</h2><p>운영시간과 통제 상태는 각 장소의 지도·공식 정보 링크에서 출발 전에 확인하세요.</p></div><div className="guide-attraction-grid">{attractions.filter(item => item.verified).map((item, index) => <article className={index < 2 ? "featured" : ""} key={item.id}><div className="guide-attraction-head"><span>{item.category}</span><b>공식 정보 연결</b></div><h3>{item.name}</h3><p>{item.summary}</p><dl><div><dt>추천 계절</dt><dd>{item.seasons.join(" · ")}</dd></div><div><dt>주요 경험</dt><dd>{item.experience}</dd></div><div><dt>이용 계획</dt><dd>{item.duration}</dd></div><div><dt>이동 경로</dt><dd>{item.distance}</dd></div><div><dt>위치</dt><dd>{item.address}</dd></div><div><dt>예약·운영</dt><dd>{item.reservation}</dd></div></dl><footer><a href={item.mapUrl} target="_blank" rel="noopener noreferrer">지도 보기 <ExternalLink size={14} /></a><a href={item.officialUrl} target="_blank" rel="noopener noreferrer">공식 정보 <ExternalLink size={14} /></a></footer></article>)}</div><p className="guide-source-note"><Info size={17} />열목어마을 주변 관광지는 <a href={guideSources.village} target="_blank" rel="noopener noreferrer">홍천군 관광정보</a>와 공공기관 자료를 기준으로 정리했습니다.</p></div></section>
    ...
    <section className="guide-section guide-map" id="map" aria-labelledby="map-title"><div className="container guide-map-layout"><iframe className="guide-map-frame" src={siteConfig.googleMapEmbedUrl} title="강원특별자치도 홍천군 내면 명개로 98 지도" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /><div><p className="section-kicker">DESTINATION</p><h2 id="map-title">열목어마을로 찾아오시는 길</h2><p>내비게이션에는 아래 대표 주소를 입력하세요.</p><ul><li><MapPinned /><span>{siteConfig.address}</span><b>대표 방문 주소</b></li></ul><div className="guide-map-actions"><a className="button button-primary" href={siteConfig.mapUrl} target="_blank" rel="noopener noreferrer">네이버 지도에서 검색 <ExternalLink size={16} /></a><CopyAddressButton address={siteConfig.address} /><Link className="button button-outline" href="/village/location">교통 안내 보기</Link></div></div></div></section>
    ...
    <section className="guide-section guide-faq" aria-labelledby="guide-faq-title"><div className="container"><div className="guide-heading"><p className="section-kicker">FAQ</p><h2 id="guide-faq-title">방문 전에 자주 묻는 질문</h2><p>교통·시설 이용 조건은 공식 정보와 실제 문의 경로를 함께 안내합니다.</p></div><div className="guide-faq-list">{guideFaqs.map(([question, answer]) => <details key={question}><summary>{question}<CircleHelp /></summary><p>{answer}</p></details>)}</div><div className="guide-related"><Link href="/village">마을 이야기</Link><Link href="/programs">프로그램</Link><Link href="/stay">숙소</Link><Link href="/group">단체·워크숍</Link></div></div></section>

    <section className="guide-final" aria-labelledby="guide-final-title"><Image src="/images/village-main-hero.jpg" alt="홍천 열목어마을 건물과 잔디마당 전경" fill sizes="100vw" /><div className="guide-final-shade" /><div className="container"><p>여행의 시작은 멀리 가는 것이 아니라 잠시 멈추는 것입니다</p><h2 id="guide-final-title">이번 주말,<span>홍천에서 제대로 쉬어보세요</span></h2><b>방문 인원과 희망 날짜, 원하는 숙박과 프로그램을 알려주시면 여행 계획을 안내해 드립니다.</b><div><Link className="button button-light" href="/contact?type=trip">여행 일정 상담</Link><Link className="button button-ghost" href="/stay">숙소 확인하기</Link><Link className="button button-ghost" href="/programs">프로그램 보기</Link><Link className="button button-ghost" href="/contact?type=group">단체 견적 문의</Link></div></div></section>
