...
export default function LocationPage() { return <VillagePageFrame eyebrow="LOCATION & VISIT" title="산길 끝에서 만나는 조용한 마을" description="정확한 목적지를 확인하고, 산간 지역의 날씨와 교통 여건을 살핀 뒤 여유 있게 출발해 주세요." image="/images/village-arrival.png" imageAlt="산과 나무에 둘러싸인 홍천 열목어마을 방문 시설">
  <section className="village-location"><div className="container">
    <div className="location-primary">
      <article>
        <p className="section-kicker">OFFICIAL ADDRESS</p>
        <h2>홍천 열목어마을</h2>
        <address><MapPin aria-hidden="true" />{villageOfficial.address}</address>
        <a className="location-phone" href={villageOfficial.phoneHref}><Phone aria-hidden="true" /><span><small>방문·운영 문의</small><b>{villageOfficial.phoneDisplay}</b></span></a>
        <p className="location-source">주소와 전화는 홍천군 문화관광포털 및 2026년 홍천군 공식 안내를 기준으로 확인했습니다.</p>
      </article>
      <div className="location-map">
        <iframe
          src={villageOfficial.googleEmbed}
          width="100%"
          height="100%"
          style={{ border: 0, display: "block" }}
          title="홍천 열목어마을 위치 지도"
          loading="eager"
    ...
    <div className="location-map-actions" role="group" aria-label="지도 보기와 주소 복사">
      <a href={villageOfficial.naverMap} target="_blank" rel="noopener noreferrer" className="button button-primary">네이버지도에서 보기 <ExternalLink size={16} aria-hidden="true" /></a>
      <a href={villageOfficial.kakaoMap} target="_blank" rel="noopener noreferrer" className="button button-outline">카카오맵에서 보기 <ExternalLink size={16} aria-hidden="true" /></a>
      <CopyAddressButton address={villageOfficial.address} />
    </div>
    <div className="location-guides"><article><Car /><h2>자가용 방문</h2><p>내비게이션에서 ‘홍천 열목어마을’ 또는 도로명 주소를 검색하세요. 산간 도로의 기상과 노면 상태는 달라질 수 있으므로 출발 전 확인하고 여유 있게 이동해 주세요.</p></article><article><Bus /><h2>대중교통</h2><p>노선과 시간표가 변경될 수 있어 이 페이지에서 고정 시간표를 안내하지 않습니다. 홍천군 버스 정보와 지도 앱에서 당일 경로를 확인하고, 마지막 구간 이동 방법은 마을에 미리 문의해 주세요.</p></article><article><CircleParking /><h2>주차 안내</h2><p>차량 종류와 대수를 방문 전에 알려주시면 주차 위치와 진입 동선을 안내합니다. 대형버스는 반드시 사전 상담해 주세요.</p></article></div>
