...
export default function ProgramsPage() {
  return <PageLayout eyebrow="HEALING PROGRAMS" title="내 몸의 속도를 되찾는 시간" description="숲·계곡·농촌·밥상을 엮어 지금의 나에게 필요한 쉼을 고릅니다." image="/images/quiet-time-3.png">
    <Section kicker="프로그램" title="누구와, 언제 오시나요?" intro="대상과 계절에 따라 활동 강도와 순서를 조정합니다."><div className="program-list">{programs.map((program) => <Link href={`/programs/${program.slug}`} className="program-row" key={program.slug}><Image src={program.image} alt={program.slug === "98-staywork" ? "홍천 열목어마을 숲과 계곡에서 일하고 쉬는 98 스테이워크" : program.title} width={720} height={520}/><div><p className="meta">{program.category ? `${program.category} · ` : ""}{program.audience} · {program.duration ?? program.season}</p><h3>{program.title}</h3><p>{program.summary}</p><ul>{program.highlights.slice(0,3).map(item=><li key={item}><Check size={16}/>{item}</li>)}</ul><span className="text-link">자세히 보기 <ArrowRight size={17}/></span></div></Link>)}</div></Section>
