...
export function SiteFooter() {
  return (
    <footer className="site-footer"><div className="container footer-grid">
      ...
      <div><h2>정보</h2><Link href="/privacy">개인정보처리방침</Link><Link href="/terms">이용약관</Link><Link href="/village/location">찾아오시는 길</Link></div>
      <div className="footer-social"><h2>열목어마을 소식</h2><div className="footer-social-links">{siteConfig.socials.map((social) => <a key={social.id} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={`${social.name} 공식 채널 새 탭에서 열기`}><SocialLogo platform={social.id} /><span>{social.name}</span></a>)}</div></div>
